...
- **Parameter Identification Algorithm** (core content of this project)

### 2.1 Parameter Identification Workflow
The identification process is performed in three sequential steps:
1.  Stator Resistance ($R_s$) Identification: Motor at standstill
2.  Rotor Resistance ($R_r$) and Leakage Inductance ($L_\sigma$) Identification: Motor at standstill
3.  Mutual Inductance ($L_m$) and No-Load Current ($I_0$) Identification: Motor running at no-load
...
**Formula**:
$$ R_s = \frac{U_{DC} \times D}{I} \times \frac{1}{2} $$

| Symbol | Description | Unit |
|:------:|:------------|:----:|
| $U_{DC}$ | DC bus voltage | V |
| $D$ | PWM duty cycle | - |
| $I$ | Measured phase current | A |
...
7.  Calculate stator resistance using the two duty cycles:
    $$ D_1 = \frac{ComperL - RS\_PRD/2}{RS\_PRD/2}, \quad D_2 = \frac{Comper - RS\_PRD/2}{RS\_PRD/2} $$
8.  Disable PWM, reset variables, and return to initial state.

### 3.2 Rotor Resistance ($R_r$) and Leakage Inductance ($L_\sigma$) Identification
**Principle**: When a step voltage is applied, since $L_m \gg L_\sigma$, the magnetizing current can be neglected. The current rise rate is determined by the total leakage inductance.

**Formula**:
$$ u_s = i_s(R_s + R_r) + 2L_\sigma \frac{di_s}{dt} $$

Solve the following two equations simultaneously:
$$ \frac{2}{3}U_{dc} = I_u(U_s=U_{dc}) \times (R_s + R_r) + 2L_\sigma \times \frac{dI_u(U_s=U_{dc})}{dt} $$
$$ 0 = I_u(U_s=0) \times (R_s + R_r) + 2L_\sigma \times \frac{dI_u(U_s=0)}{dt} $$
...
- The motor must be at standstill for $R_s$ and $R_r/L_\sigma$ identification steps.
