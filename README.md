...
Stator Resistance (Rs) Identification: Motor at standstill
Rotor Resistance (Rr) and Leakage Inductance (Lσ) Identification: Motor at standstill
Mutual Inductance (Lm) and No-Load Current (I0) Identification: Motor running at no-load


3. Detailed Identification Algorithms
3.1 Stator Resistance (Rs) Identification
Principle: DC volt-ampere method. Apply DC excitation between two stator phases, measure voltage and current to calculate Rs.
Formula:R_s =(U_DC * D)/I  * 1/2
...
2.Enable PWM output, delay 2s (for continuous identification).
...
7.Calculate stator resistance using the two duty cycles:D_1 = (ComperL - RS_PRD/2)/(RS_PRD/2), D_2 = (Comper - RS_PRD/2)/{RS_PRD/2)
...
Principle: When a step voltage is applied, L_m >> Lσ, so magnetizing current can be neglected. The current rise rate is determined by the total leakage inductance.
Formula:
...
Formula:L_m = U_m * I_m * sin(theta) /I_m^2  * 1/2*pi*f - Lσ

Where:
U_m: Fundamental voltage amplitude
I_m: Fundamental current amplitude
theta: Phase difference between voltage and current
f: Fundamental frequency

Implementation Steps:

Initialize variables, target frequency: 80% of motor rated frequency.
Enable PWM output, delay 1.6s.
Wait until actual output frequency reaches 80% of rated frequency.
Perform 120 consecutive identifications (240ms interval), average the results.
Abort identification if motor speed drops below the set threshold.
...
Rated Parameters: 3AC220V, 50Hz, 26A
...
5. Repository Structuretext              
├── models/               # Simulink simulation models
└── README.md             # This document


6. Getting Started

Configure the VFD hardware parameters (rated voltage, current, frequency).
Run the automatic parameter identification sequence from the VFD keypad.
Verify the identified parameters against the motor nameplate values.
Use the identified parameters for vector control tuning.
